...
protected:
    ...
    {
        // Small Arrowhead Ship Shape
        vecModelShip = { 
            { 0.0f, -2.5f }, // Nose (Index 0)
            {-1.5f, +1.5f }, 
            { 0.0f, +0.5f }, 
            {+1.5f, +1.5f }  
        };
        ...
        // Generate Deep Space Starfield Background
        for (int i = 0; i < 100; i++) {
    ...
    // EDGE COLLISION BUG FIX: Accounts for screen wrap-around distances!
    bool IsPointInsideCircle(float cx, float cy, float radius, float x, float y)
    {
        float dx = fabs(x - cx);
        float dy = fabs(y - cy);

        // If distance is greater than half the screen, the shortest path is wrapping around the edge!
    ...
    virtual bool OnUserUpdate(float fElapsedTime)
    {
        ...
        if (m_keys['W'].bHeld) {
            ...
            vecParticles.push_back({
                ...
                player.dx * -0.2f + (float)((rand() % 20) - 10), 
        ...
        for (auto &a : vecAsteroids) {
            ...
            // GREY ASTEROIDS: Made grey per your request
        ...
        for (auto &b : vecBullets) {
            ...
            for (auto &a : vecAsteroids) {
                if(IsPointInsideCircle(a.x, a.y, a.nSize, b.x, b.y)) {
                    ...
                    // Will break down to size 2, then size 1, then disappear
                    if (a.nSize > 1) {
        ...
        // Draw Player Ship (Base White)
        DrawWireFrameModel(vecModelShip, player.x, player.y, player.angle, 1.0f, FG_WHITE);
        
        // RED NOSE: Calculate the exact coordinate of the ship's nose and paint it Red!
